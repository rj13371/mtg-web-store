    ...
    const login = await axiosClient({
      ...
      .then((response) => {
        ...
        if (response.status === 201) {
          ...
          setMessage(
            `Please check your email at ${loginFormEmail} to verify your account, ${loginFormUserName}`
          );
          setMessageCount(messageCount + 1);
      ...
      .catch((e) => {
        if (e) {
          ...
          setMessage(
            ` ${loginFormEmail} or ${loginFormUserName} is already in use, please register with a different username and or email`
          );
          setMessageCount(messageCount + 1);
        }
      });
        ...
        <Row>
          <InputGroup size="sm" className="mb-3">
            <InputGroup.Text id="inputGroup-sizing-sm">
              {isValidCaptcha ? "Valid ✓ " : "Captcha ❌"}
            </InputGroup.Text>
            <Form.Control
              onChange={handleFormCaptcha}
              value={formCaptcha}
              type="text"
              placeholder="captcha"
            />
