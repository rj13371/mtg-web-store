...
export default function DecklistsDashboard(props) {
  ...
  const [formdecklist, handleformDecklistChange, isValidDecklist] =
    useInputState([]);
  ...
  const handleSubmit = (e) => {
    ...
    if (
      res[0].name.toLowerCase().includes("mainboard") &&
      res.some((obj) => obj.name.toLowerCase() == " sideboard")
    ) {
      setDecklist(res);

      setHeader("Please confirm before submitting");
      setMessage(<pre>{JSON.stringify(res, null, 2)}</pre>);
      setMessageCount(messageCount + 1);
    } else {
      setHeader("Invalid Decklist");
      setMessage(
        "Please include a Mainboard Quantity at the top and a Sideboard quantity within the textfield"
      );
      setMessageCount(messageCount + 1);
    }
  };
      ...
      <Row>
        <Col md={8}>
          <Form onSubmit={(e) => handleSubmit(e)}>
            ...
            {props.employee ? (
              <Form.Group className="mb-3" controlId="formemail">
                <Form.Label>
                  Enter Record for Customer Example:3-0 (update all when event
                  completed){" "}
                </Form.Label>
                <Form.Control
                  type="text"
        ...
        <Col md={4}>
          ...
          <Card style={{ color: "black" }}>
            ...
            <Card.Body>
              60 Mainboard <br></br>4 Delver of Secrets <br></br>2 Snapcaster
              Mage <br></br>4 Lightning Bolt <br></br>
              15 Sideboard <br></br>4 Lightning Helix <br></br>3 Negate{" "}
              <br></br>
            </Card.Body>
          </Card>
          ...
          {decklist.length == 0 ? null : (
            <Button onClick={decklistSubmit}>Submit Decklist to Event</Button>
          )}
        </Col>
      </Row>
    </Container>
