...
module.exports.createDecklist = async (req, res) => {
  ...
  if (req.body.email) {
    ...
    foundEntrant = event.entrants.find(
      (user) => user.username == req.body.username
    );
  }

  if (foundEntrant)
    return res.status(409).json({ message: "user already entered!" });
  ...
  try {
    ...
    return res
      .status(201)
      .send(`Successful Submission: ${newDecklist} Event: ${event} `);
  } catch (e) {
...
module.exports.editDecklist = async (req, res, next) => {
  ...
  try {
    const editedDecklist = await Decklist.findByIdAndUpdate(id, {
